var CACHE = 'uap-v21-article-header-layout';
...
function withFeedOverrides(resp) {
  ...
  return resp.text().then(function(html) {
    if (html.indexOf('app-feed-overrides.js') === -1) {
      html = html.replace('</body>', '<script src="./app-feed-overrides.js?v=21"></script></body>');
    } else {
      html = html.replace(/app-feed-overrides\.js\?v=\d+/g, 'app-feed-overrides.js?v=21');
    }
...
self.addEventListener('fetch', function(e) {
  ...
  if (url.pathname.endsWith('/app-feed-overrides.js')) {
