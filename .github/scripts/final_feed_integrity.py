...
def title_candidates(article: dict[str, Any], include_related: bool = True) -> list[str]:
...
def title_bridge_same_story(a: dict[str, Any], b: dict[str, Any], include_related: bool = True) -> bool:
    for a_title in title_candidates(a, include_related=include_related):
        for b_title in title_candidates(b, include_related=include_related):
...
def same_story_article(a: dict[str, Any], b: dict[str, Any]) -> bool:
    if (article_has_thin_context(a) or article_has_thin_context(b)) and title_bridge_same_story(a, b, include_related=False):
        return True
...
def same_story_source(article: dict[str, Any], source: dict[str, Any]) -> bool:
    ...
    if article_has_thin_context(article) and title_bridge_same_story(article, source_article, include_related=False):
        return True
