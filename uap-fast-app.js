(function(){
  'use strict';
  if (window.__uapFastApp) return;
  window.__uapFastApp = true;
  ...
  function titleKey(value){ return compact(value).toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim(); }
  ...
  function articleTime(article){ return parseDateValue(article && (article.publishedAt || article.date || article.displayedAt)); }
  function displayTime(article){ return parseDateValue(article && (article.displayedAt || article.firstDisplayedAt || article.detectedAt || article.createdAt || article.timestamp || article.publishedAt || article.date)); }
  function sourceTime(source){ return parseDateValue(source && (source.displayedAt || source.sourceDisplayedAt || source.firstDisplayedAt || source.detectedAt || source.createdAt)); }
  function recent(time){ var age = Date.now() - time; return !!time && age >= 0 && age <= NEW_WINDOW_MS; }
  function isExplicitNewSource(source, article, primary){
    ...
    return recent(primary ? parseDateValue(article && (article.sourceDisplayedAt || article.displayedAt)) : sourceTime(source));
  }
  ...
  function isNewArticle(article){
    ...
    var shown = displayTime(article);
    if (!shown || !recent(shown)) return false;
    var published = parseDateValue(article.publishedAt || article.date);
    if (published && !recent(published) && shown - published > NEW_WINDOW_MS) return false;
    return true;
  }
  ...
  function sourceKey(source){ return key(source && (source.link || source.url || source.source + '|' + source.title)); }
  function sourcesFor(article){
    var sources = [];
    if (article && (article.link || article.url)) sources.push({ source: article.source || 'Quelle', link: article.link || article.url, title: article.title || '', isNew: !!article.sourceIsNew, displayedAt: article.sourceDisplayedAt || article.displayedAt });
    (article && article.otherSources || []).forEach(function(source){
      if (source && (source.link || source.url)) sources.push({ source: source.source || 'Quelle', link: source.link || source.url, title: source.title || '', isNew: !!source.isNew, displayedAt: source.displayedAt || source.sourceDisplayedAt });
    });
    var seen = {};
    return sources.filter(function(source){
      var k = sourceKey(source);
  ...
  function findArticleById(id){ return (state.feed.articles || []).filter(function(article){ return articleId(article) === id; })[0] || null; }
  ...
  function pointsNumber(value){ var match = String(value || '').match(/-?\d+/); return typeof value === 'number' ? value : (match ? Number(match[0]) : 0); }

  function normalizeFeed(payload){
    if (!payload || !Array.isArray(payload.articles)) return { articles: [] };
    var byTitle = {}, order = [];
    payload.articles.forEach(function(raw){
      if (!raw || !compact(raw.title)) return;
      var article = Object.assign({}, raw);
      article.otherSources = sourcesFor(article).slice(article.link || article.url ? 1 : 0);
      ...
      var k = titleKey(article.title);
      if (!byTitle[k]) { byTitle[k] = article; order.push(k); return; }
      var target = byTitle[k];
      var mergedSources = sourcesFor(target).concat(sourcesFor(article));
      target.otherSources = mergedSources.filter(function(source, index){ return index > 0; });
      target.mentions = Math.max(Number(target.mentions || 1), Number(article.mentions || 1), 1 + target.otherSources.length);
      if (compact(article.summary).length > compact(target.summary).length) target.summary = article.summary;
      if (parseQuality(article) > parseQuality(target)) {
        target.quality = parseQuality(article);
        target.sourceQuality = parseQuality(article);
        if (article.qualityBreakdown) target.qualityBreakdown = article.qualityBreakdown;
      }
    });
    var articles = order.map(function(k){ return byTitle[k]; }).sort(function(a,b){
      var q = parseQuality(b) - parseQuality(a);
      return q || articleTime(b) - articleTime(a);
    });
    return Object.assign({}, payload, { articles: articles });
  }

  function injectStyle(){
    if (document.getElementById('uap-fast-app-style')) return;
    var css = [
      ...
      '.article-card{cursor:pointer;transform:translateZ(0);overflow-anchor:none!important;border:1px solid rgba(13,58,92,.95);background:linear-gradient(180deg,rgba(8,20,32,.98),rgba(5,15,23,.98));position:relative;overflow:hidden}',
      '.article-card.uap-detail-open{contain:layout paint style!important}.article-card h2{color:#eef9fd!important;font-family:system-ui,-apple-system,"Segoe UI",Arial,sans-serif!important;font-weight:700!important;letter-spacing:0!important;text-transform:none!important;text-shadow:none!important}',
      '.article-main{display:block;width:100%;text-align:left;border:0;background:transparent;color:inherit;padding:14px 16px 12px 18px;cursor:pointer;overflow-anchor:none!important;touch-action:manipulation!important}',
  ...
  function ensureStaticControls(){
    ...
    if (row && !document.getElementById('notify-btn')) {
      var notify = document.createElement('button');
      notify.type = 'button';
      notify.id = 'notify-btn';
      notify.className = 'icon-btn notify-btn';
      notify.setAttribute('aria-label', 'Push-Benachrichtigungen über ntfy öffnen');
      notify.innerHTML = '<svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9"/><path d="M10 21h4"/><path d="M4 4l16 16"/></svg>';
      row.insertBefore(notify, row.firstChild);
    }
    var feed = document.getElementById('feed');
    if (feed && feed.parentNode && !document.querySelector('.quality-top-help')) {
      var q = document.createElement('button');
      q.type = 'button';
      q.className = 'quality-top-help';
      q.innerHTML = '<span class="quality-info-dot">i</span><span>Wertung</span>';
      feed.parentNode.insertBefore(q, feed);
    }
    if (feed && feed.parentNode && !document.getElementById('uap-new-filter-bar')) {
      var bar = document.createElement('div');
      bar.id = 'uap-new-filter-bar';
  ...
  function sourceListHtml(article){
    return sourcesFor(article).map(function(source){
      var fresh = isExplicitNewSource(source, article, false) ? '<span class="badge source-new-badge">New</span>' : '';
      var title = source.title && source.title !== article.title ? '<div class="source-headline">' + esc(source.title) + '</div>' : '';
      return '<a class="source-link" href="' + esc(source.link) + '" target="_blank" rel="noopener noreferrer"><div class="source-name">' + esc(source.source || 'Quelle') + fresh + '</div>' + title + '</a>';
    }).join('');
  }
  function articleHtml(article, index){
    ...
    return '<article class="' + cardClass + '" data-uap-id="' + esc(id) + '" data-uap-order="' + index + '" data-uap-quality="' + quality + '" data-uap-sort-time="' + articleTime(article) + '">' +
  ...
  function renderFeed(keepOpenId){
    var feedEl = document.getElementById('feed');
    if (!feedEl) return;
    var openId = keepOpenId || (feedEl.querySelector('.article-card.uap-detail-open') || {}).dataset && (feedEl.querySelector('.article-card.uap-detail-open') || {}).dataset.uapId;
    ...
    if (openId) setArticleOpen(feedEl.querySelector('.article-card[data-uap-id="' + CSS.escape(openId) + '"]'), true);
  }
  ...
  function toggleCard(card){ setArticleOpen(card, !card.classList.contains('uap-detail-open')); }
  function interactiveTarget(target){ return closest(target, 'a,input,select,textarea,button:not(.article-main),.badge.quality,.translate-btn,#notify-btn,.quality-overlay,.notify-guide-overlay,.source-list,.quality-top-help,#uap-new-filter-toggle'); }
  function cardFromEvent(event){ if (event.__uapHandled || interactiveTarget(event.target)) return null; return closest(event.target, '.article-card'); }
  function stopEvent(event){ event.preventDefault(); event.__uapHandled = true; if (event.stopImmediatePropagation) event.stopImmediatePropagation(); else event.stopPropagation(); }

  function toggleTranslation(card){
    var id = card && card.dataset.uapId;
    var article = findArticleById(id);
    if (!article) return;
    if (state.translated[id]) { delete state.translated[id]; renderFeed(id); return; }
    var prepared = choosePrepared(article);
    if (!prepared) {
      var btn = card.querySelector('.translate-btn');
      if (btn) { btn.textContent = 'Übersetzung noch nicht bereit'; setTimeout(function(){ if (btn.textContent === 'Übersetzung noch nicht bereit') btn.textContent = 'Übersetzen'; }, 2200); }
      return;
    }
  ...
  function qualityRows(article){
    var rows = (article && Array.isArray(article.qualityBreakdown) ? article.qualityBreakdown : []).filter(Boolean).map(function(row){ return { label: compact(row.label || 'Wertung'), points: row.points, text: compact(row.text || '') }; });
    var score = parseQuality(article);
    if (!rows.length) rows = [{ label:'Basis', points: Math.min(27, score), text:'Grundwertung, weil der Artikel als UAP/UFO-relevant erkannt wurde.' }, { label:'Relevanz', points: Math.max(0, score - 27), text:'Zusätzliche Punkte aus Begriffen, Quelle, Thema und Quellenanzahl.' }];
    var sum = rows.reduce(function(total, row){ return total + pointsNumber(row.points); }, 0);
    var diff = score - sum;
    if (diff > 0) rows.push({ label:'Priorisierung', points: diff, text:'Zusätzliche Anhebung durch die finale GitHub-Bewertung.' });
    if (diff < 0) rows.push({ label:'Deckelung', points: diff, text:'Die Rohpunkte wurden begrenzt.' });
    return rows;
  }
  function generalQualityRows(){
    var map = {};
    (state.feed.articles || []).forEach(function(article){ (article.qualityBreakdown || []).forEach(function(row){
      if (!row || !row.label) return;
      var label = compact(row.label), point = pointsNumber(row.points);
      if (!map[label]) map[label] = { label: label, min: point, max: point, count: 0, text: compact(row.text || '') };
      map[label].min = Math.min(map[label].min, point); map[label].max = Math.max(map[label].max, point); map[label].count += 1; if (!map[label].text) map[label].text = compact(row.text || '');
    }); });
    var rows = Object.keys(map).map(function(label){ return map[label]; }).sort(function(a,b){ return b.max - a.max || b.count - a.count; }).slice(0, 9);
    if (!rows.length) rows = [{ label:'Mehrere Quellen', min:0, max:40, text:'Mehr Quellen erhöhen die Priorität, wenn sie zum gleichen Thema gehören.' }, { label:'Quelle', min:0, max:14, text:'Offizielle Quellen und etablierte Nachrichtenmedien werden stärker gewichtet.' }];
    return rows.map(function(row){ return { label: row.label, points: row.min === row.max ? pointText(row.max) : pointText(row.min) + ' bis ' + pointText(row.max).replace('+', ''), text: row.text || ('Dieser Faktor wurde bei ' + row.count + ' aktuellen Artikeln verwendet.') }; });
  }
  function closeOverlay(selector){ var old = document.querySelector(selector); if (old) old.remove(); }
  function showQuality(article, general){
    closeOverlay('.quality-overlay');
    var rows = general ? generalQualityRows() : qualityRows(article);
    var scoreLine = article ? '<div class="quality-score-line"><span>Quellen: ' + sourcesFor(article).length + '</span><strong>' + parseQuality(article) + ' Punkte</strong></div>' : '';
    var overlay = document.createElement('div');
    overlay.className = 'quality-overlay';
    overlay.setAttribute('role', 'dialog');
    overlay.setAttribute('aria-modal', 'true');
    overlay.innerHTML = '<div class="quality-sheet"><h3>' + esc(general ? 'Wertung' : 'Wertung ' + parseQuality(article)) + '</h3>' + scoreLine + '<p>Die Wertung ist eine Priorisierung für die App. Sie bewertet Relevanz und Quellenlage, nicht ob eine Behauptung wahr ist.</p><div class="quality-rules">' + rows.map(function(row){ return '<div class="quality-rule"><span class="quality-points">' + esc(pointText(row.points)) + '</span><span><strong>' + esc(row.label || 'Wertung') + ':</strong> ' + esc(row.text || '') + '</span></div>'; }).join('') + '</div><button type="button" class="quality-close">SCHLIESSEN</button></div>';
    overlay.addEventListener('click', function(e){ if (e.target === overlay || e.target.classList.contains('quality-close')) closeOverlay('.quality-overlay'); });
    document.body.appendChild(overlay);
  }
  function showNotifyGuide(){
    closeOverlay('.notify-guide-overlay');
    var overlay = document.createElement('div');
    ...
    overlay.addEventListener('click', function(e){
      if (e.target === overlay || e.target.hasAttribute('data-notify-close')) closeOverlay('.notify-guide-overlay');
      if (e.target.hasAttribute('data-notify-primary')) window.open('https://ntfy.sh/' + encodeURIComponent(NTFY_TOPIC), '_blank', 'noopener,noreferrer');
    });
    document.body.appendChild(overlay);
  }
  ...
  function bindEvents(){
    ...
    window.addEventListener('click', function(event){
      if (event.__uapHandled) return;
      var notify = closest(event.target, '#notify-btn');
      if (notify) { stopEvent(event); showNotifyGuide(); return; }
      var filter = closest(event.target, '#uap-new-filter-toggle');
      if (filter) { stopEvent(event); state.newOnly = !state.newOnly; renderFeed(); return; }
      var topHelp = closest(event.target, '.quality-top-help');
      if (topHelp) { stopEvent(event); showQuality(null, true); return; }
      var quality = closest(event.target, '.badge.quality');
      if (quality) { stopEvent(event); var c = closest(quality, '.article-card'); showQuality(findArticleById(c && c.dataset.uapId), false); return; }
      var translate = closest(event.target, '.translate-btn');
      if (translate) { stopEvent(event); if (!translate.disabled) toggleTranslation(closest(translate, '.article-card')); return; }
      var card = cardFromEvent(event);
      if (!card) return;
      stopEvent(event);
      if (Date.now() - lastPointerToggle < 650 || Date.now() - lastTouchToggle < 450) return;
      toggleCard(card);
    }, true);
    window.addEventListener('touchstart', function(event){
      if (!event.touches || event.touches.length !== 1 || interactiveTarget(event.target)) { touchStart = null; return; }
      var card = closest(event.target, '.article-card');
      if (!card) { touchStart = null; return; }
    ...
    window.addEventListener('touchmove', function(event){
      ...
      var touch = event.touches[0];
      if (Math.abs(touch.clientX - touchStart.x) > 12 || Math.abs(touch.clientY - touchStart.y) > 12) touchStart.moved = true;
    }, { capture: true, passive: true });
    window.addEventListener('touchend', function(event){
      if (!touchStart || touchStart.moved || event.defaultPrevented || interactiveTarget(touchStart.target)) { touchStart = null; return; }
      var card = touchStart.card;
      touchStart = null;
      if (!card) return;
      stopEvent(event);
    ...
    window.addEventListener('keydown', function(event){
      if (event.key === 'Escape') { closeOverlay('.quality-overlay'); closeOverlay('.notify-guide-overlay'); return; }
      var quality = closest(event.target, '.badge.quality');
      if (quality && (event.key === 'Enter' || event.key === ' ')) { stopEvent(event); var c = closest(quality, '.article-card'); showQuality(findArticleById(c && c.dataset.uapId), false); return; }
      var main = closest(event.target, '.article-main');
      if (main && (event.key === 'Enter' || event.key === ' ')) { stopEvent(event); toggleCard(closest(main, '.article-card')); }
    }, true);
  }

  function loadFeed(){
    return fetch('latest-news.json?fast=' + Date.now(), { cache: 'no-store' })
      .then(function(resp){ return resp.ok ? resp.json() : { articles: [] }; })
      .then(function(feed){ state.feed = normalizeFeed(feed); renderFeed(); })
      .catch(function(){ state.feed = { articles: [] }; renderFeed(); });
  }
  function hideStartSoon(){
    function hide(){ var loading = document.getElementById('loading'); if (loading) loading.classList.add('hidden'); }
    window.addEventListener('load', function(){ setTimeout(hide, 900); }, { once: true });
    setTimeout(hide, 1400);
  }
  function normalizeUrl(){
    try { var url = new URL(window.location.href); if (url.searchParams.has('notif') || url.searchParams.has('ids')) { url.searchParams.delete('notif'); url.searchParams.delete('ids'); window.history.replaceState({}, document.title, url.pathname + (url.search || '') + url.hash); } } catch(e) {}
  }
  function start(){
    injectStyle();
    normalizeUrl();
    ...
    if (refresh && !refresh.dataset.fastBound) { refresh.dataset.fastBound = '1'; refresh.addEventListener('click', function(){ window.location.reload(); }); }
